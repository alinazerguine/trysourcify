 ...
 * @return  mixed
 */
function royal_option( $id ) {
	static $theme_settings;

	if ( empty( $theme_settings ) ) {
		$theme_settings = apply_filters( 'royal_customize_settings', array() );
	}
 ...
 * @return  strings
 */
function royal_current_post_type() {
 ...
 * @since   1.0.0
 */
function royal_logo( $prefix = '', $attrs = array() ) {
	$logo_key = 'logo';
	...
	$logo = (array) royal_option( $logo_key );
	$logo_retina = (array) royal_option( $logo_retina_key );
	...
	printf( '<img %s />', royal_attributes( $attributes ) );
}


function royal_social_icons( $args ) {
	$args = array_merge( array(
		'location'     => 'top',
		'wrapper'      => '<div class="social-icons">%s</div>',
		'wrapper_icon' => '<i class="%s"></i>',
		'wrapper_link' => '<a href="%1$s" data-tooltip="%2$s">%3$s</a>'
	), $args );
	$positions = royal_option( 'global__social__positions' );

	if ( in_array( $args['location'], $positions ) ) {
		$icons = royal_option( 'global__social__icons' );
		$output = array();
 ...
 * @since   1.0.0
 */
function royal_attributes( $attrs ) {
	$attributes = array();
...
function royal_google_fonts() {
	...
	$cached = get_transient( 'royal-fonts' );
	...
	if ( ! $cached ) {
		...
		if ( is_array( $remote_content ) && isset( $remote_content['body'] ) ) {
			...
			set_transient( 'royal-fonts', $cached, 86400 );
		}
	}
 ...
 * @since   1.0.0
 */
function royal_initialize_filesystem_api( $url = '', $action = -1, $name = '_wpnonce' ) {
 ...
 * @return  array
 */
function royal_get_image_resized( $params = array() ) {
	...
	if ( is_string( $thumb_size ) && ( ( ! empty( $_wp_additional_image_sizes[ $thumb_size ] ) && is_array( $_wp_additional_image_sizes[ $thumb_size ] ) ) || in_array( $thumb_size, array(
		...
		if ( is_array( $thumb_size ) ) {
			...
			$p_img = royal_image_resize( $attach_id, null, $thumb_size[0], $thumb_size[1], $params['crop'] );
			...
			if ( ! empty( $attachment ) ) {
				...
				if ( $p_img ) {
					...
					$thumbnail = '<img ' . royal_attributes_stringify( $attributes ) . ' />';
				}
			}
		}
	}
	...
	return apply_filters( 'royal_get_image_resized', array(
		'thumbnail' => $thumbnail,
		'thumbnail_raw' => $p_img,
		'large' => $p_img_large,
	), $attach_id, $params );
 ...
 * @return array
 */
function royal_image_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
 ...
 * @return string
 */
function royal_attributes_stringify( $attributes ) {
	$atts = array();
 ...
 */
function royal_get_image_sizes() {
	global $_wp_additional_image_sizes;
