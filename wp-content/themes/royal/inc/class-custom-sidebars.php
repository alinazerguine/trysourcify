...
class Cls__Custom_Sidebars
{
	...
	public function enqueue_assets( $hook = null ) {
		...
		wp_localize_script( 'custom-sidebars', '_sidebarSettings', array(
			'button_title'    => __( 'remove this area', 'royal' ),
			'confirm_message' => __( 'Are you sure you want to remove this widget area?', 'royal' ),
	...
	public function register_sidebars() {
		...
		$sidebar_options = apply_filters( 'royal/custom_sidebars_params', array(
	...
	public function admin_form() {
		...
		$messages = array();
		$messages[1] = __( 'New custom widget area has been created', 'royal' );
		$messages[2] = __( 'Custom widget area has been removed', 'royal' );

		$errors = array();
		$errors[2] = __( 'Cannot create custom widget area, please try again', 'royal' );
		$errors[3] = __( 'Invalid the ID of widget area', 'royal' );
		?>
			...
			<div id="sidebars-form" class="widget-liquid-right">
				<form action="<?php echo esc_url( admin_url() ) ?>admin-ajax.php?action=save_custom_sidebar" method="POST" class="widgets-holder-wrap">
					...
					<h3><?php _e( 'Create Widget Area', 'royal' ) ?></h3>
					<div class="sidebar-inputs">
						<label for="sidebar-name"><?php _e( 'Enter name of the widget area:', 'royal' ) ?></label>
						<input type="text" name="sidebar-name" id="sidebar-name" />
						<input type="submit" class="button button-primary" value="<?php _e( 'Create', 'royal' ) ?>" />
					</div>
				</form>
				...
				<?php if ( ! empty( $custom_sidebars ) ): ?>
					<form id="remove-sidebar-form" action="<?php echo esc_url( admin_url() ) ?>admin-ajax.php" method="GET" class="widgets-holder-wrap hide-if-js">
						...
						<h3><?php _e( 'Remove Custom Widget Area', 'royal' ) ?></h3>
						<div class="sidebar-inputs">
							<label for="sidebar-id"><?php _e( 'Select custom widget area to be removed:', 'royal' ) ?></label>
							...
							<input type="submit" class="button" value="<?php _e( 'Remove', 'royal' ) ?>" />
						</div>
					</form>
				<?php endif ?>
			</div>
	...
	public function save_sidebar() {
		if ( isset( $_POST['custom-sidebar'] ) &&
			...
			$custom_sidebars['sidebar-' . uniqid()] = array(
				'name' => $name,
				'description' => sprintf( __( 'There are widgets for %s', 'royal' ), $name )
			);
