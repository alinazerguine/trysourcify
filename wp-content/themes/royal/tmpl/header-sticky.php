...
$header_classes[] = sprintf( 'header-brand-%s', royal_option( 'header__sticky__logoAlign' ) );

if ( royal_option( 'header__sticky__width' ) === 'on' ) {
	$header_classes[] = 'header-full';
}

if ( royal_option( 'header__sticky__shadow' ) === 'on' ) {
	$header_classes[] = 'header-shadow';
}

if ( royal_option( 'header__sticky__transparent' ) === 'on' ) {
	$header_classes[] = 'header-transparent';
}
?>

	<div id="site-header-sticky" class="wrap <?php echo esc_attr( join( ' ', $header_classes ) ) ?>">
		<div class="site-header-inner wrap">
			...
			<div class="header-brand">
				<a href="<?php echo esc_attr( site_url() ) ?>" alt="<?php bloginfo( 'site_name' ) ?>">
					<?php royal_logo( royal_option( 'header__sticky__logo' ) ) ?>
				</a>
			</div>
			...
			<?php if ( has_nav_menu( 'primary' ) || has_nav_menu( 'secondary' ) ): ?>
				<nav class="navigator" itemscope="itemscope" itemtype="http://schema.org/SiteNavigationElement">
					...
					<?php royal_social_icons( array( 'location' => 'sticky' ) ) ?>
